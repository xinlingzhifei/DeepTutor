...
`openai-codex` 会打开系统浏览器，使用 yFeiSTAI 自己的独立 OAuth 流程登录。它不需要 `OPENAI_API_KEY`，也不会读取或同步本机 `~/.codex`；凭据保存在 `<user-root>/private/openai-codex/`，与 Web 设置页共用。授权回调监听本机回环地址，因此须在运行后端的机器上完成登录。
