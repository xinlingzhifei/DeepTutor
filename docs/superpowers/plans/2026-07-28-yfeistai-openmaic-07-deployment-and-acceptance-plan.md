...
这 8 个非 Playwright receipt producer 的本地实现进度为：`capacity_profile` 与 `learning_event_idempotency` 已实现并提交；`classroom_exports` 的 producer、原始导出合同与 verifier replay 已在当前候选分支实现并纳入本批，且尚无 live receipt。`tenant_isolation` 的固定 probe、严格 report contract、candidate/run/capacity 绑定、receipt producer 与 verifier replay 也已在当前候选分支完成本地实现；它覆盖 database、objects、exports、events 四层隔离，并显式证明 owner 在 foreign tenant 中仅具 active student membership，避免把 admission 失败误报为租户谓词隔离。其当前本地证据为 evidence/verifier 四文件 `288 passed / 2 skipped`，以及身份、租户 API、probe/contract 五文件 `149 passed / 1 warning`；这两条命令存在重叠，不得相加。本地主机 Docker daemon 只读预检超时，因此本地 Testcontainers 仍是 setup INCOMPLETE；同一工作树归档经 SHA-256 校验后在隔离服务器运行器取得 PostgreSQL/Testcontainers `7 passed`、native exit 0，且临时 PG/Ryuk 残留为 0、既有业务容器身份集合前后不变。该服务器门禁证明当前源码的真实 PostgreSQL 隔离与 membership/grant 级联，不是同一已部署发布候选的 live receipt。`openmaic_shared_plane` 的本地 producer 现已实现：单一 platform-admin token 通过正式 active-tenant API 创建隔离 teacher fixture，等待异步 outline 后确认并等待 content materialization，读取 admin-only 持久 binding，严格绑定 classroom/version/DSL、Content-Length、ETag 与文档 SHA-256；probe 会扫描 admin token、teacher 密码与真实 teacher session，writer 使用 no-follow 目录租约、隔离 staging、identity 校验与 proof-last 发布，verifier 重放 candidate/releaseRun/runtime/command/stdout/summary。`openmaic_dedicated_plane`、`backup_restore` 与 `gateway_only_public` 的本地 probe、receipt producer 和 verifier replay 也已在当前工作树实现；当前批次的最终静态检查已覆盖 90 个变更 Python 文件且 Ruff、格式、语法、workflow YAML 与 diff-check 全部通过，隔离服务器上的完整 Python/PostgreSQL 门禁为 `1541 passed / 1 warning`、native exit 0，既有容器身份集合前后不变且本批运行器残留为 0。三项仍均未取得同一已部署候选的 live receipt。上述本地实现和离线 replay 均不代表同一已部署不可变发布候选已取得 live receipt，也不代表 Task7 或 Task8 已完成。

**`openmaic_shared_plane` fixture 清理边界：** probe 先确认高熵 teacher username 不存在，再在每个可逆写请求发出前登记单一复合补偿 intent；服务端提交但响应丢失时通过正式用户清单按精确 username 恢复唯一普通用户 ID。成功或异常退出均按 enrollment → tenant membership/grants → local identity 的依赖逆序撤销；删除允许一次有界重试，只接受精确 absent tombstone，identity 与 membership 使用 compare-and-delete，任一子级撤销、身份漂移或响应合同失败都会 fail closed 并停止删除父级。tenant、course、class、append-only quota grant、published classroom/version/job/binding 无完整安全删除合同，保留为 candidate/run-bound 审计 fixture，不得报告为“完整 teardown”。SIGKILL、断电或宿主崩溃仍可绕过进程内补偿栈；这是需要 durable fixture ownership ledger/reaper 或服务端 lease/TTL 加固的 P2 运维边界，不得宣称崩溃后零残留。本地 MockTransport GREEN 只证明补偿控制流和 HTTP 合同，真实环境仍须以同一候选的 live receipt 与运行后残留核验收口。
