...
普通浏览器业务请求的活跃租户只从服务端设置的 `dt_tenant` Cookie 解析；没有
Cookie 时，仅允许自动选择唯一有效成员关系对应的租户。租户切换只能经过受控的
`PUT /api/v1/tenants/active`，由服务端校验成员关系和租户状态后更新 Cookie。
普通请求中的 `X-Tenant-ID` 不可信，不能建立或覆盖活跃租户；它与非空 Cookie
冲突时必须 fail-closed。
...
之间只通过私有网络调用，并使用独立、签名的服务身份；租户必须绑定在签名声明
或签名请求中，不复用用户 JWT、`dt_tenant` Cookie 或其他用户票据，也不把机构
Provider 密钥放入任务消息。
...
- 普通 API 从已验证登录态和服务端 `dt_tenant` Cookie 解析租户，不接受
  `X-Tenant-ID` 或其他客户端自报 `tenant_id` 切换租户。
