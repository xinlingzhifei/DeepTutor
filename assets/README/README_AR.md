...
**OpenAI Codex OAuth (تجريبي).** اختيار **OpenAI Codex** ضمن **النماذج ← LLM** يستبدل حقول مفتاح API بتسجيل دخول عبر المتصفح يعمل مقابل خطة ChatGPT الخاصة بك، لذا لا حاجة إلى `OPENAI_API_KEY`. تعيش الرموز (tokens) فقط في `<user-root>/private/openai-codex/`، ولا يقرأ DeepTutor أو يعدّل أبداً تسجيل دخولك إلى CLI في `~/.codex`. تأتي قائمة النماذج من الكتالوج الحي لذلك الحساب؛ تسجيل الدخول ينشر الملف الشخصي لكنه لا يصبح النموذج النشط إلا عندما لا يكون هناك LLM مهيأ بعد، لذا لا يعيد توجيه عملية نشر خلف ظهرك أبداً. ولأن الرمز يخوّل خطة شخص واحد فقط، فالملف الشخصي غير قابل للمشاركة عبر منح المستخدمين — كل حساب يسجّل دخوله بنفسه، ويجب أن يصل المتصفح إلى الجهاز الذي يشغّل الخلفية (على خادم بعيد، شغّل `deeptutor provider login openai-codex` هناك بدلاً من ذلك). تُبلَّغ أخطاء الحصة وفشل الكتالوج كما هي ولا تتراجع أبداً إلى مزود مدفوع. مسار التوافق هذا تجريبي: قد تتغيّر واجهة المصدر الأعلى (upstream).
