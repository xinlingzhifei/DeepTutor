# yFeiSTAI v1.5.7 Release Notes

**Release Date:** 2026.07.31

A follow-up to [v1.5.6](ver1-5-6.md) that opens the tutor up to tools yFeiSTAI never shipped: a curated store of hosted MCP services each person installs for themselves, and 101 command-line apps from the CLI-Anything catalog the chat agent can call directly. Both arrive with the authorisation and isolation a shared deployment actually needs — installing is a privileged action, running is not, and nothing that authorises a person stays anywhere the code sandbox can read. Drop-in — no migrations.
