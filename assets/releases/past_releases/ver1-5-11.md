...
- **Connected knowledge bases answer honestly about local files.** A linked external resource has no yFeiSTAI-managed `raw/` directory; listing its files returns an empty collection instead of inventing one, and endpoints that require a local file return an explicit `409`.
