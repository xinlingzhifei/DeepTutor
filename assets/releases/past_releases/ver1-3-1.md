...
- **Selected KB is now trusted system state** - chat RAG calls no longer let the
  LLM invent or override `kb_name`; the model only sees a `query`, and yFeiSTAI
...
- **Embedding responses are validated before use** - yFeiSTAI now rejects
