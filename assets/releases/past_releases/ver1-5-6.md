...
v1.5.5 shipped OpenAI Codex OAuth with a caveat — on a remote box, sign in from the server instead. That caveat is gone. The browser's `localhost` and the server's `localhost` are different machines, so a reverse proxy alone can never carry the callback; yFeiSTAI now forwards only the exact callback path through the Web origin to a state-validating broker, and the settings card and CLI print the real callback address, the SSH tunnel command, an explicit authorize action, and a countdown that survives a page reload ([#716](https://github.com/HKUDS/DeepTutor/issues/716)).
...
- **Remote Codex sign-in now uses an SSH tunnel**, and the v1.5.5 advice to run `deeptutor provider login openai-codex` on the server is no longer the only path. yFeiSTAI prints the exact `ssh -N -L …` command; run only the one matching the callback port it reports, and note that `3782` in that command is an example — replace it with the Web port actually listening on the SSH host's loopback.
- A Tencent IMA knowledge base needs a `client_id` and API key from [ima.qq.com/agent-interface](https://ima.qq.com/agent-interface) plus the library id. Documents are added in IMA, not through yFeiSTAI.
