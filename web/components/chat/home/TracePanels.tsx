...
function getTraceHeader(
  ...
  if (
    ...
    // top-level status row carries the verbose "yFeiSTAI Reflecting…"
...
function detectStreamingMode(
  ...
  for (let idx = events.length - 1; idx >= 0; idx -= 1) {
    ...
    if (event.type === "content" && callKind === "llm_final_response") {
      ...
      // still open; keep the top-level title on "yFeiSTAI Exploring…" until
      // the bus moves on.
      if (event.stage === "exploring") return "exploring";
...
export function StreamingStatus({
  ...
  const name = agentName?.trim() || "yFeiSTAI";
  ...
  else if (mode === "responded") modeLabel = t("yFeiSTAI responded.");
 ...
 * reasoning trace once yFeiSTAI stops working and starts (or has finished)
 * its answer.
 *
 ...
 * ("yFeiSTAI Exploring… · 8s", settling to "yFeiSTAI responded. · 10s")
 * with the exploring trace nested directly beneath it.
 *
 * The trace is expanded by default while yFeiSTAI is still reasoning /
