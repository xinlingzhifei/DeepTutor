...
export async function runTeacherClassroomFlow({
  page,
  teachingDownload,
}: {
  page: Page;
...
export async function runTeacherExportMatrix({ page }: { page: Page }) {
  test.setTimeout(180_000);
  await page.addInitScript(() => {
    localStorage.setItem("deeptutor-language", "en");
  });
  ...
  await page.goto(`/teaching/classrooms/${ASSET_ID}/edit`);
  await expect(
    page.getByRole("heading", { name: "Energy Transfer Lab" }),
  ).toBeVisible({ timeout: 60_000 });

  const formats = ["classroom_zip", "pptx", "offline_html", "mp4"] as const;
  for (const format of formats) {
    const responseOk = await page.evaluate(
      async ({ assetId, exportFormat }) => {
        const response = await fetch(
          `/api/v1/classrooms/${assetId}/draft/exports`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              "If-Match": '"revision-3"',
              "Idempotency-Key": `release-export-${exportFormat}`,
            },
      ...
      { assetId: ASSET_ID, exportFormat: format },
    );
    expect(responseOk).toBe(true);
  }

  expect(state.exportCalls.map(call => call.body)).toEqual(
    formats.map(format => ({ format })),
  );
  expect(
    state.exportCalls.map(call => call.headers["if-match"]),
  ).toEqual(formats.map(() => '"revision-3"'));
  expect(
    state.exportCalls.map(call => call.headers["idempotency-key"]),
  ).toEqual(formats.map(format => `release-export-${format}`));
  expect(state.unexpected).toEqual([]);
}
